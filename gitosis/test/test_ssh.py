...
class GenerateAuthorizedKeys_Test(object):
    def test_simple(self):
        ...
        eq(gen.next(), (
            'command="PATH=/opt/local/bin gitosis-serve jdoe",no-port-forwarding,no-X11-f'
            +'orwarding,no-agent-forwarding,no-pty %s' % KEY_1))
        eq(gen.next(), (
            'command="PATH=/opt/local/bin gitosis-serve wsmith",no-port-forwarding,no-X11'
...
class FilterAuthorizedKeys_Test(object):
    ...
    def test_notFiltered_keyWithCommand(self):
        s = '''\
command="PATH=/opt/local/bin faketosis-serve wsmith",no-port-forwarding,no-X11-forwardin\
...
''' % dict(key_1=KEY_1)
    ...
    def test_filter_simple(self):
        s = '''\
command="PATH=/opt/local/bin gitosis-serve wsmith",no-port-forwarding,no-X11-forwardin\
g,no-agent-forwarding,no-pty %(key_1)s
''' % dict(key_1=KEY_1)
        got = self.run(s)
        eq(got, '')
    ...
    def test_filter_withPath(self):
        s = '''\
command="PATH=/opt/local/bin /foo/bar/baz/gitosis-serve wsmith",no-port-forwarding,no-X11-forwardin\
g,no-agent-forwarding,no-pty %(key_1)s
''' % dict(key_1=KEY_1)
        got = self.run(s)
        eq(got, '')
