...
def isSafeUsername(user):
    ACCEPTABLE_USER_RE = compile(r'^[a-zA-Z][a-zA-Z0-9_.-]*(@[a-zA-Z][a-zA-Z0-9.-]*)?$')
    match = ACCEPTABLE_USER_RE.match(user)
...
def readKeys(keydir):
    ...
    """
    for filename in listdir(keydir):
...
def filterAuthorizedKeys(fp):
    ...
    """
    COMMAND_RE = compile('^command="PATH=/opt/local/bin (.*gitosis-serve) [^"]+",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty .*')


    for line in fp:
        line = line.rstrip('\n')
        if line != COMMENT or not COMMAND_RE.match(line):
            yield line
